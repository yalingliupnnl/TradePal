...
if __name__ == "__main__":
    ...
    for sym in symbols:
        log = pd.DataFrame(columns=['n_units',
                                   'batch_size',#4500
                                   'ln_rate',
                                   'train_loss',
                                  'val_loss',
        ...
        for i in np.arange(len(n_units)):
            for j in np.arange(len(batch_size)):
                for k in np.arange(len(ln_rate)):
                    ...
                    model_loss = myANN(ANN_params,symbol = sym, sd=sd, 
                    ...
                    count +=1
                    if count%10 ==0:
                        print('Finished running the '+ str(count)+ 'th search for index fund '+sym+', keep going!')
                        print(dt.datetime.now())
         
        end_time=dt.datetime.now()
        use_time=end_time - start_time
        use_minutes=(use_time.seconds+use_time.microseconds/1000000)/60.0  
        print('time used for grid searching:  '+str(use_minutes)+ ' minutes!')
        log.to_csv('tradepal/models/ANN_grid_search_'+sym+'.csv', sep=',')
    
        #save the best hyperparameters
        test_acc=log['test_acc'].values
        max_acc_idx=np.argmax(test_acc)
        
        #append best params for each symbol
        params = params.append({'symbol'         : sym,
        ...
        print('Finished seeking best hyperparameters for '+sym+ '!')
    params.to_csv('tradepal/models/ANN_best_para.csv', sep=',')
